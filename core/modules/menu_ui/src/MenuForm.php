...
class MenuForm extends EntityForm {
    ...
    public static function create(ContainerInterface $container) {
        return new static(
                $container->get('plugin.manager.menu.link'), $container->get('menu.link_tree'), $container->get('link_generator')
        );
    }
        ...
        foreach ($tree as $element) {
            ...
            if ($link) {
                ...
                if (!$link->getParent()) {
                    ...
                    $cls = $link->getTitle();
                    $clss1 = 'menu-enabled parentclass ' . $cls;
                    $clss2 = 'menu-enabled childclass ' . $cls;
                    $clss3 = 'menu-enabled parentclass ' . $cls;
                    $clss4 = 'menu-enabled childclass ' . $cls;
                }


                if (!$link->getParent()) {
                    $form[$id]['#attributes'] = $link->isEnabled() ? ['class' => ["menu-enabled parentclass '".$cls."'"]] : ['class' => ["menu-disabled parentclass '".$cls."'"]];
                } else {
                    $form[$id]['#attributes'] = $link->isEnabled() ? ['class' => ["menu-enabled childclass '".$cls."'"]] : ['class' => ["menu-disabled childclass '".$cls."'"]];
                }
